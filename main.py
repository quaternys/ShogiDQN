...
def sfen_to_move(board, turn, sfen) -> tuple:
    ...
    n1 = 9 * i1 + j1    # 移動先
    if sfen[1] == "*":  # 打ち
        return n1, PIECES_SFEN.find(sfen[0]), None, None, None, None
...
if __name__ == "__main__":
    ...
    while True:
        cmd = input().split()
        if cmd[0] == "usi":
            print("id name ♥♥♥どきゅんがどっきゅ〜ん♥♥♥")
            print("option name DepthLimit type spin default 1 min 1 max 8")
            print("option name Temperature type spin default 0.01 min 0 max 2")
            print("usiok")
        elif cmd[0] == "isready":
            print("readyok")
